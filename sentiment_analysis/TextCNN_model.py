...
'''代码参考https://blog.csdn.net/sinat_34604992/article/details/103078205'''
#################################################################################
############################## 定义各种参数、路径 #################################
...
class Config(object):
    def __init__(self, dataset_dir, embedding):
        ...
        self.train_path = './data/train/data/virus_train.txt'  # 训练集
        self.dev_path = './data/train/data/virus_dev.txt'  # 验证集
        self.test_path = './data/train/data/virus_test.txt'  # 测试集
        self.usual_test_path = './data/eval/data/usual_test.txt' # 待测试数据集
        self.virus_test_path = './data/eval/data/virus_realtest.txt'
        ...
        with open(path, 'r', encoding='UTF-8') as f:
            for line in tqdm(f):
                ...
                for word in token:
                    ...
                    #print(words_line)
                d = {'angry': '0', 'happy': '1', 'neural': '2', 'surprise': '3', 'sad': '4', 'fear': '5'}
...
def build_dataset2(config, ues_word):
    ...
    path = "./data/eval/data/virus_realtest.txt"
    test = load_dataset(config.usual_test_path, config.pad_size)
    #print(train)
    return vocab, test
...
class DatasetIterater2(object):
    ...
    def _to_tensor(self, datas):
        ...
        return (x, seq_len)  # (x[b, l], l[b]), y[b]
...
vocab_dir = "./data/train/vocab.pkl"
...
#测试
#test_dir = "./data/eval/data/usual_test.txt"
model.load_state_dict(torch.load(config.save_path))
model = model.eval()
vocab, usual_test_data = build_dataset2(config, use_word)
usual_test_iter = build_iterator2(usual_test_data, config) 

label_to_file = []
#获取模型输入，拼接成label数组
for texts in usual_test_iter:
    #print(texts)
    outputs = model(texts) 
    predic = torch.max(outputs.data, 1)[1].cpu().numpy()
    label_to_file += list(predic)
    
#print(label_to_file)
sentiments = []
for item in label_to_file:
    if item == 0:
...
#######写入文件########
df_usual_eval = pd.read_excel('./data/eval/virus_eval.xls')
submission_df = pd.DataFrame(data={'id': df_usual_eval['数据编号'].values, 'label': sentiments})
